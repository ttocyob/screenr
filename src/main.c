...
/* Hand-rolled entry point instead of the ELM_MAIN() macro so --version/-v
 * can be answered before elm_init() touches X, config or the theme -- the
 * macro's expansion is essentially this minus the argv scan (see
 * elm_general.h: elm_init(); elm_main(); elm_shutdown()). SCREENR_VERSION
 * comes from screenr_version.h, generated by meson vcs_tag() (git describe,
 * or the project_version() fallback for a .git-less tree). */
int
   ...
   for (int i = 1; i < argc; i++)
     {
        if (!strcmp(argv[i], "--version") || !strcmp(argv[i], "-v"))
          {
             puts("screenr " SCREENR_VERSION);
             return 0;
          }
     }

   elm_init(argc, argv);
   int ret = elm_main(argc, argv);
   elm_shutdown();
   return ret;
}
